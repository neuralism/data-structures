...
for (var i = 0; i < 10; i++) {
    var fileTitle = i < 9 ? 'm0' + (i + 1) : 'm' + (i + 1);
    var content = fs.readFileSync('/home/ubuntu/workspace/week6/data/' + fileTitle + '.txt');
    ...
    $('tbody tr').each(function(i, elem) {
        ...
        for (var i = 0; i < data.length; i++) {
            ...
            if (data[i] !== '') {
                ...
                meeting.address = $(elem).find('td').eq(0).html().split('\n')[3].split(',')[0].split('- ')[0].split('(')[0].split('@')[0].trim() + ', New York, NY';
                ...
                // Assign meeting types
                if (text.split('Meeting Type')[1] !== undefined)
...
async.eachSeries(meetings, function(value, callback) {
    ...
    fs.writeFileSync('/home/ubuntu/workspace/week6/output.txt', JSON.stringify(meetings));
});